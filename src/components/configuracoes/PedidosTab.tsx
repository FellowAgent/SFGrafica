...
import { Plus, Save, GripVertical, User, Minus, ArrowRight } from "lucide-react";
...
interface SortableVendedorProps {
  vendedor: VendedorFluxo;
...
function SortableVendedor({ vendedor, onToggleAtivo, editingId, setEditingId, onUpdateNome }: SortableVendedorProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <Switch
              ...
              onCheckedChange={(checked) => onToggleAtivo(vendedor.id, checked)}
              className={cn(
          ...
          <div className="flex items-center justify-center mb-3">
            <Avatar className="h-12 w-12">
              <AvatarImage 
                src={vendedor.avatar_url ? `${vendedor.avatar_url}?t=${Date.now()}` : undefined} 
                className={cn(
                  "object-cover",
                  !vendedor.ativo && "grayscale"
                )}
              />
              <AvatarFallback className="bg-muted">
                {vendedor.nome ? (
                  vendedor.nome.split(' ').slice(0, 2).map(n => n[0]).join('').toUpperCase()
                ) : (
          ...
          {editingId === vendedor.id ? (
            <Input
              autoFocus
              value={vendedor.nome}
              onChange={(e) => onUpdateNome(vendedor.id, e.target.value.toUpperCase())}
            ...
            <h3 
              ...
              onClick={() => setEditingId(vendedor.id)}
            >
...
function SortableStatus({ status, index, onToggleAtivo, isOverlay = false }: SortableStatusProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export function PedidosTab() {
  ...
  const { status: statusConfig, loading: loadingStatus, updateStatus } = useStatusConfig();
  const [vendedores, setVendedores] = useState<VendedorFluxo[]>([]);
  ...
  // Carregar fluxo de vendedores do banco de dados
  useEffect(() => {
  ...
  const carregarFluxoVendedores = async () => {
    try {
      ...
      // Ordenar por ordem
      vendedoresCompletos.sort((a, b) => a.ordem - b.ordem);
      setVendedores(vendedoresCompletos);
    } catch (error) {
      console.error("Erro ao carregar fluxo de vendedores:", error);
      toast.error("Erro ao carregar configuração de vendedores");
    }
  };

  // Funções para gerenciar vendedores
  const toggleVendedorAtivo = (id: string, ativo: boolean) => {
    ...
    supabase
      ...
      .upsert({
        user_id: id,
        ativo: ativo,
        ordem: vendedores.find(v => v.id === id)?.ordem || 0,
      }, {
        onConflict: 'user_id'
      })
      .then(({ error }) => {
  ...
  const atualizarNomeVendedor = (id: string, novoNome: string) => {
    setVendedores(vendedores.map(v => v.id === id ? { ...v, nome: novoNome } : v));
  };

  const handleDragEndVendedores = (event: DragEndEvent) => {
    const { active, over } = event;
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Visualize as etapas do kanban de pedidos. Para editar, vá até a aba "Aparência" &gt; "Status de Pedidos".
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {loadingStatus ? (
            ...
            >
              <div className="flex items-center justify-center gap-4 flex-wrap">
                <SortableContext
                  items={statusOrdenados.map(s => s.id)}
                  strategy={horizontalListSortingStrategy}
                >
                  {statusOrdenados.map((status, index) => (
                    <div key={status.id} className="flex items-center gap-4">
                      ...
                      {/* Seta conectora */}
                      {index < statusOrdenados.length - 1 && (
        ...
        >
          {loadingUsuarios ? (
            ...
            <DndContext
              sensors={sensors}
              collisionDetection={closestCenter}
              onDragEnd={handleDragEndVendedores}
            >
              <div className="flex items-center justify-center gap-4 flex-wrap">
                <SortableContext
                  items={vendedores.map(v => v.id)}
                  strategy={horizontalListSortingStrategy}
                >
                  {vendedores.map((vendedor, index) => (
                    <div key={vendedor.id} className="flex items-center gap-4">
                      <SortableVendedor
                        vendedor={vendedor}
                        onToggleAtivo={toggleVendedorAtivo}
                        editingId={editingVendedorId}
                        setEditingId={setEditingVendedorId}
                        onUpdateNome={atualizarNomeVendedor}
                      />

                      {/* Linha conectora */}
                      {index < vendedores.length - 1 && (
