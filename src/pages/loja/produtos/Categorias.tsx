...
const Categorias = () => {
  ...
  }, [modalFromUrl, categoriaIdFromUrl, categorias]);

  const handleEditarCategoria = (categoria: any) => {
    navigate(`/loja/produtos/categorias?modal=editar&categoriaId=${categoria.id}`);
  };

  const handleAbrirNovaCategoria = () => {
    navigate("/loja/produtos/categorias?modal=novo");
  };

  const handleFecharModal = () => {
    navigate("/loja/produtos/categorias", { replace: true });
    setShowForm(false);
    setEditandoCategoria(undefined);
    fetchCategorias();
  };
            ...
            <div className="rounded-2xl border bg-card shadow-fellow-md">
              ...
              >
                <Table>
                  ...
                  <SortableContext
                    items={categoriasAchatadas.map(c => c.id)}
                ...
                <DragOverlay>
                  {activeId ? (
                    <div className="bg-card border-2 border-primary rounded-lg p-4 shadow-2xl animate-scale-in min-w-[300px]">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-muted-foreground flex items-center gap-2">
                            <span className="bg-muted px-2 py-0.5 rounded">
                              Nível {acharCategoria(categorias, activeId)?.nivel || 0}
                            </span>
                            {acharCategoria(categorias, activeId)?.subcategorias?.length > 0 && (
                              <span className="text-primary">
                                {acharCategoria(categorias, activeId)?.subcategorias.length} subcategoria(s)
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="mt-2 text-xs text-muted-foreground italic border-t pt-2">
                        Arraste para reposicionar na hierarquia
                      </div>
                    </div>
                  ) : null}
                </DragOverlay>
