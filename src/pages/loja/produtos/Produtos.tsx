...
const Produtos = () => {
  ...
  const handleEditarProduto = (produto: any) => {
    navigate(`/loja/produtos?modal=editar&produtoId=${produto.id}`);
  };

  const handleAbrirNovoProduto = () => {
    navigate("/loja/produtos?modal=novo");
  };

  const handleFecharModal = () => {
    navigate("/loja/produtos", { replace: true });
    setShowForm(false);
    setEditandoProduto(null);
    fetchProdutos();
  };
                        ...
                        >
                          ...
                          <TableCell>
                            <Avatar className="h-10 w-10 rounded-md">
                              <AvatarImage src={produto.imagem_url} loading="lazy" />
                              <AvatarFallback className="rounded-md bg-accent">
                ...
                >
                  <div className="aspect-square bg-accent/20 flex items-center justify-center">
                    <Avatar className="h-24 w-24 rounded-md">
                      <AvatarImage src={produto.imagem_url} loading="lazy" />
                      <AvatarFallback className="rounded-md bg-accent text-2xl">
                        {produto.nome.substring(0, 2).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                  </div>
