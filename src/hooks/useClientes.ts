...
export const useClientes = () => {
  ...
  const fetchClientes = async (
    ...
    try {
      ...
      // Não mostrar erro se for problema de autenticação
      if (!error.message?.includes('JWT') && !error.message?.includes('permission denied')) {
        toast({ title: "Erro", description: "Erro ao carregar clientes: " + error.message, variant: "destructive" });
      }
      console.warn('Clientes fetch error:', error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const createCliente = async (input: unknown) => {
    try {
      ...
      toast({ title: "Erro", description: "Erro ao cadastrar cliente: " + error.message, variant: "destructive" });
      throw error;
    }
  };
  ...
  const updateCliente = async (id: string, input: unknown) => {
    try {
      ...
      toast({ title: "Erro", description: "Erro ao atualizar cliente: " + error.message, variant: "destructive" });
      throw error;
    }
  };
